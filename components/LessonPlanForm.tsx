...
const LOCAL_STORAGE_KEY = 'lessonPlanHistory';
...
const LessonPlanForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const response = await execute(() => generateLessonPlan(subject, grade, topic));
    
    if (response) {
      setDisplayedResult(response);
      const newHistoryItem: HistoryItem = {
        id: Date.now(),
        inputs: { subject, grade, topic },
        result: response,
        timestamp: Date.now(),
      };
    ...
    <div className="animate__animated animate__fadeIn">
      <div className={`bg-white p-8 rounded-lg shadow-lg border border-gray-200/75 transition-all duration-300 ${isSuccess ? 'ring-2 ring-green-500 ring-offset-2' : ''}`}>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <label htmlFor="topic" className="block text-sm font-semibold text-gray-800 mb-2">Tema Principal</label>
            <textarea id="topic" value={topic} onChange={(e) => setTopic(e.target.value)} required rows={3} className="block w-full px-4 py-2 bg-white border border-gray-300 rounded-lg shadow-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-brand-secondary/50 focus:border-brand-secondary hover:border-gray-400 transition-all duration-200 sm:text-sm" placeholder="Ej: Introducción a las fracciones" />
          </div>
          <div>
